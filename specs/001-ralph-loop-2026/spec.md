...
**Independent Test**: Can be tested by installing a sample plugin (e.g., @gyasis/micro-agent-plugin-prettier), configuring it to run onAfterGen hook, generating code, and verifying plugin executes and modifies output (formats code). Delivers extensibility value for power users.

**Acceptance Scenarios**:

1. **Given** ralph-plugins.yaml with "@gyasis/micro-agent-plugin-prettier" enabled, **When** code generation completes, **Then** plugin's onAfterGen hook executes, code is auto-formatted, and formatted code is written to file
2. **Given** "@gyasis/micro-agent-plugin-security-scan" plugin configured for onBeforeSuccess hook, **When** tests pass, **Then** plugin runs semgrep and snyk scans, and blocks success if critical vulnerabilities found
