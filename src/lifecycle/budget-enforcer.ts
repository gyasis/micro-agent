...
export function calculateBudgetUtilization(
  ...
  const iterationPct = (budgetStatus.currentIteration / config.maxIterations) * 100;
  const costPct = (budgetStatus.totalCost / config.maxCostUsd) * 100;
  const durationPct = (budgetStatus.elapsedMinutes / config.maxDurationMinutes) * 100;
...
export function estimateRemainingBudget(
  ...
  const remainingIterations = config.maxIterations - budgetStatus.currentIteration;
  const remainingCostUsd = config.maxCostUsd - budgetStatus.totalCost;
  const remainingMinutes = config.maxDurationMinutes - budgetStatus.elapsedMinutes;

  const canContinue = remainingIterations > 0 && remainingCostUsd > 0 && remainingMinutes > 0;
...
export function formatBudgetStatus(
  ...
  lines.push(`${statusIcon} Overall Utilization: ${utilization.overall.toFixed(0)}%`);
...
export function predictBudgetOverrun(
  ...
  const projectedDuration = budgetStatus.elapsedMinutes + averageIterationDuration;
